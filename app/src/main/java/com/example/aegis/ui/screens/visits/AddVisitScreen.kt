...
fun AddVisitScreen(
    ...
    ) { uris: List<Uri> -> if (uris.isNotEmpty()) vm.onFilesPicked(uris) }

    if (showDatePicker) {
        val datePickerState = rememberDatePickerState(
            initialSelectedDateMillis = vm.visitDate,
        )
        DatePickerDialog(
                ...
                OutlinedTextField(
                    ...
                    trailingIcon = {
                        if (filteredSuggestions.isNotEmpty())
                            ExposedDropdownMenuDefaults.TrailingIcon(expanded = providerDropdownExpanded && filteredSuggestions.isNotEmpty())
                    },
                    modifier = Modifier
                        .fillMaxWidth()
                        .menuAnchor(),
